...
var create_user = function (data) {
  return new Promise(function (resolve, reject) {
    ...
    if (CONFIG[secion].db_name_append_guid) {
      dbname.push(uuid.v4());
    }
...
var server = http.createServer(function (req, resp) {
  ...
  create_user(data)
    ...
    .then(function (d) {
      ...
      if (CONFIG[section].cors_support) {
        resp.writeHead(200, { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Access-Control-Request-Method': '*', 'Access-Control-Allow-Methods': 'POST,GET', 'Access-Control-Allow-Headers': '*' });
      } else {
